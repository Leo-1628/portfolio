# Leo S

## Software Developer | Python Developer

### About Me
Aspiring Software Developer with expertise in Python, SQL, Object-Oriented Programming, Machine Learning, and Database Management.

### Education
- B.Tech Computer Science and Engineering
- MBA (Information Technology) - Pursuing

### Technical Skills
- Python
- SQL
- Java
- C
- Git & GitHub
- MySQL
- Machine Learning
- NumPy
- Matplotlib

### Certifications
- Fundamentals of Python (NPTEL)
- Introduction to Cloud Computing (Infosys Springboard)

### Featured Project
#### Brain Stroke Prediction and Detection Using Machine Learning
Machine learning-based system for stroke prediction and classification using CT scan images.

### Contact
- LinkedIn: linkedin.com/in/leo-s-win16
- Email: leo161028@gmail.com# portfolio
Professional portfolio showcasing software development projects, technical skills, certifications, and achievements.
